...
func _animate_state(active: bool) -> void:
	...
	var target_y := -1.5 if active else 0.0
	_motion_tween.tween_property(self, "scale", target_scale, 0.13).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	_motion_tween.tween_property(self, "position:y", target_y, 0.13).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
...
func _animate_pressed() -> void:
	...
	_motion_tween.tween_property(self, "position:y", 0.5, 0.06)
