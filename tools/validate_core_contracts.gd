...
func _validate_operations_screen() -> void:
	...
	_expect('Button/styles/pressed = SubResource("ButtonDark")' in theme_source, "Button press states must keep identical geometry.")
...
func _validate_flight_screen() -> void:
	...
	var flight_source := FileAccess.get_file_as_string("res://src/ui/screens/flight/flight_screen.gd")
	_expect("_input_service.is_pointer_boost_event(event)" in flight_source, "Flight must route desktop world clicks before GUI can consume them.")
	_expect("_is_pointer_over_flight_ui" in flight_source, "Flight must explicitly guard HUD regions from world-click boost.")
	_expect("touch_controls.is_pointer_over_boost_hud" in flight_source, "Boost HUD clicks must never fall through into world-click boost.")
	_expect("_operations_overlay != null" in flight_source and "_travel_in_progress" in flight_source, "World-click boost must be disabled while menus or travel own input.")
