...
func _validate_flight_screen() -> void:
	...
	_expect("navigation.configure(ship, depot)" in flight_source, "Flight must bind depot navigation to real ship/depot world nodes.")
	...
	_expect("get_canvas_transform()" in guide_source, "Depot guide must project the world target through the active camera canvas transform.")
	_expect("NEAR_DISTANCE" in guide_source, "Depot guide must disappear near the depot instead of remaining invasive.")
