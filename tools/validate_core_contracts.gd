...
func _validate_operations_screen() -> void:
	...
	_expect('Button/styles/pressed = SubResource("ButtonDarkPressed")' in theme_source, "Dark buttons must keep explicit pressed feedback without changing label contrast.")
	_expect('PrimaryButton/styles/hover = SubResource("ButtonYellowHover")' in theme_source, "Primary yellow actions must use a surface hover state.")
...
func _validate_flight_screen() -> void:
	...
	_expect('HudActionButton/styles/hover = SubResource("HudButtonActionHover")' in flight_theme_source, "Flight action hover must be visualized by the surface, not text recoloring.")
