...
- The live-flight HUD deliberately uses a darker companion theme for contrast over gameplay, while keeping the same fonts, spacing discipline and Kenney-derived action language. The current-flight and cargo status cards use near-black surfaces, bright neutral borders, white/light typography and a restrained original Kenney glass-notch frame; cyan/mint/amber are reserved for meaningful status and progress accents. The Operations action uses the original full blue Kenney bar so it reads as clickable as clearly as red Abort Contract and green Complete Contract.
